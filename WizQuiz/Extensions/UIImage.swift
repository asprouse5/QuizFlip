...
extension UIImage {
    ...
    internal class func delayForImageAtIndex(_ index: Int, source: CGImageSource!) -> Double {
        ...
        // Get dictionaries
        let cProperties = CGImageSourceCopyPropertiesAtIndex(source, index, nil)
        let gifPropertiesPointer = UnsafeMutablePointer<UnsafeRawPointer?>.allocate(capacity: 0)
        if CFDictionaryGetValueIfPresent(cProperties,
    ...
    internal class func gcdForPair(_ one: Int?, _ two: Int?) -> Int {
        var one = one
        var two = two
